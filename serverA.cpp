...
string checkOverlap(char* users, unordered_map<string, list<list<int>>> myMap, list<list<int>>& overlap){
  ...
  cout << "Found the intersection result: " << overlapString << " for " << users << "." << endl;
 ...
 * @param the socket number for the current server
 */
void notifyServer(sockaddr_in serverMAddr, int sockfd, string selectedUsers, char* selectedTime){
  socklen_t serverMAddrLen = sizeof(serverMAddr);  
  ...
  if (sendResult == -1) cout << "recvResult" << endl;
    ...
    if (s != "empty" && str != "[]")  cout << "Notified Main Server that registration has finished." << endl;
  }
}
/**
...
int main(){
  ...
  while (true){
    ...
    notifyServer(serverMAddr, sockfd, selectedUsers, selectedTime);
  }
  close(sockfd);
  return 0;
}